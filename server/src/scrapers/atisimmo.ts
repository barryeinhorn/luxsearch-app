...
const SOURCE = 'atisimmo' as const;
const BASE_URL = 'https://www.atisimmo.lu';

function buildUrl(params: SearchParams): string {
  const tx = params.transaction === 'sale' ? 'sale' : 'rent';
  return `${BASE_URL}/en/search?transaction=${tx}`;
}

export async function scrapeAtisimmo(params: SearchParams): Promise<Property[]> {
